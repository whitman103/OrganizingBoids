...
struct Agent
{
    ...
    virtual void update_pos(const vector<unique_ptr<Agent>> &nearbyAgents);
    Agent(Agent &&other) noexcept : position(std::move(other.position)), velocity(std::move(other.velocity)){};
};
